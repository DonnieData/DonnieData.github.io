...
## Data
Our data is in JSON (JavaScript Object Notation) format, which is an array containg key-value pairs. Each segment of the array contains the same "keys" with different "values" which is our ufo sighting data. Becuase each section is the same "shape" and shares the same keys, it will be easy to transform and display the data into a tabular format.
